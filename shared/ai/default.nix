{
  ...
  ennio,
  nix-everywhere,
  # Which optional agent tooling to install. Set per home in flake.nix:
  # true on bebop, false on the disk-capped cloud guests. Not `? true`
  # defaults — a module-arg default is not honoured on this nixpkgs, so
  # every home must pass these explicitly.
  installForge, # Forge refetches crates and overflows a capped guest
  installEnnio, # ennio-node is on the cloud hosts' system profile already
  installLmstudio, # LM Studio is a desktop GUI, wrong for an agent guest
  ...
}:
let
...
in
{
  home = {
    ...
    packages =
      ...
      ++ lib.optionals installForge [ forgecode.packages.${system}.default ]
    ...
    file =
      claudeFiles
      // (lib.optionalAttrs installForge forgeFiles)
      // piFiles
      // codexFiles
      // grokFiles
      // agentsSkillsFiles
