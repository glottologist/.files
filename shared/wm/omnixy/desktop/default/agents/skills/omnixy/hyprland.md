...
Omnixy configures Hyprland in Lua. User files are loaded after Omnixy's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads Omnixy defaults, then user files)
...
- Use `omnixy refresh hyprland` to reset the Lua config files to defaults

The two `.conf` files are read by separate processes, so `hyprctl` neither
applies nor validates them:
- `hyprsunset.conf` (night light): apply changes with `omnixy restart hyprsunset`; reset with `omnixy refresh hyprsunset`
...
View current bindings: `omnixy menu keybindings --print`

**IMPORTANT: When re-binding an existing key:**

1. First check existing bindings: `omnixy menu keybindings --print`
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer Omnixy's `o.window(match, rules)` helper — see examples in `$OMNIXY_PATH/default/hypr/windows.lua`.
