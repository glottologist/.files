# Reporting a Crash Upstream to Omnixy

Read this only after concluding that a crash is genuinely Omnixy's to fix.

## Is it even Omnixy's bug?

Be strict here. Omnixy is a configuration layer over Arch Linux, so a crash
inside a third-party application — a file manager, a browser, a GNOME or Qt
library — is almost always an upstream bug in **that** project, not in Omnixy.

Omnixy's sphere of control is roughly:
...
A crash in a program Omnixy merely installs is **not** an Omnixy bug unless
Omnixy's own packaging or configuration is implicated.

If it is not Omnixy's, say so and stop. Suggesting the right upstream project is
useful; filing there yourself is not part of this.

## Three conditions, all required

1. **It is a verified bug in Omnixy's sphere**, established on evidence. Issues
...
`omnixy version`, and diagnostics from `omnixy debug --no-sudo --print` (which
also writes `/tmp/omnixy-debug.log`; the interactive `omnixy debug` can upload
