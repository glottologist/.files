...
return function(kind)
  -- Hardcoded to ~/.local/state to match omnixy-toggle-input-device and the
  -- sibling bash toggle tools, which all write there regardless of
  -- XDG_STATE_HOME.
  local file = io.open(paths.home .. "/.local/state/omnixy/toggles/hypr/" .. kind .. "-disabled-name", "r")
  if not file then
    return
  end
