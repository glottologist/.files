...
local function command_from(value, description)
  ...
  if value.omnixy then
    return "omnixy-launch-" .. value.omnixy
  ...
  elseif value.tui then
    if value.focus then
      return "omnixy-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "omnixy-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.preinstalled_bindings_enabled()
  if _G.omnixy_preinstalled_bindings ~= nil then
    return _G.omnixy_preinstalled_bindings == true
  end

  return not file_exists((os.getenv("HOME") or "") .. "/.local/state/omnixy/preinstalls-removed")
end
...
function o.launch_webapp(url)
  return "omnixy-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "omnixy-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "omnixy-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "omnixy-toggle-" .. toggle, options)
end

function o.notify(message)
  return "omnixy-notification-send -u low " .. shell_quote(message)
end
