...
`omnixy-shell` is a single long-running [Quickshell](https://quickshell.org/)
instance that hosts the Omnixy desktop. Hyprland autostart launches one shell
...
| `bar`        | A full bar option that can replace the built-in `omnixy.bar` |

Only one `bar` plugin is active at a time. Missing or invalid selections fall
back to the built-in `omnixy.bar`, so users always have a safe path home.
...
clones it straight into `~/.config/omnixy/plugins/<id>/` (named by the
manifest id); updating is a fast-forward pull of that checkout.

```bash
omnixy plugin add https://github.com/acme/omarchy-weather.git
omnixy plugin update acme.weather       # fetches, shows a diff, fast-forwards
omnixy plugin update                    # updates every git-managed plugin
omnixy plugin remove acme.weather
```

> ⚠️ **Plugins run as unsandboxed code inside `omnixy-shell`.** Adding warns
...
```bash
omnixy plugin add https://github.com/acme/omarchy-weather.git --enable --yes
omnixy plugin update --yes
```
...
1. Put it in `~/.config/omnixy/plugins/<plugin-id>/` with a `manifest.json`
   plus the QML referenced from its `entryPoints`.
2. `omnixy-shell shell rescanPlugins`.
3. `omnixy plugin enable <id>`. Bar widgets start in
   `barWidget.defaultSection`, or in the center when it is omitted, and can be
   moved with `omnixy bar move`; a full bar replaces the one in use.

The lower-level IPC equivalents remain available via `omnixy-shell shell rescanPlugins`,
`omnixy-shell shell enablePlugin <id> '{}'`, and `omnixy-shell shell listPlugins`.
The `omnixy plugin` commands wrap those calls. `omnixy bar move` and
`omnixy bar set` edit the persisted widget layout in `shell.json`.
...
automatically; `omnixy-shell shell rescanPlugins` remains available to force a reload.
...
hooks, the image picker's `image-selector` target). `omnixy-menu` uses the
shell target to summon the first-party `omnixy.menu` plugin instead of
...
| `reloadConfig`                           | `ok`    | reload `~/.config/omnixy/shell.json`                 |
...
```
quickshell ipc -p $OMNIXY_PATH/shell call shell ping
```

Hyprland autostart launches the shell directly with `quickshell -p
$OMNIXY_PATH/shell`. Use `omnixy-restart-shell` to stop every running
instance of that config and launch one fresh shell process.

A convenience wrapper, [`omnixy-shell`](../bin/omnixy-shell), forwards IPC
calls to the running shell. It does not start the shell.

```
omnixy-shell shell ping
omnixy-shell shell toggle omnixy.menu '{"menu":"root"}'
omnixy-shell shell listPlugins
...
| Path                              | Owner          | Purpose                                                |
|-----------------------------------|----------------|--------------------------------------------------------|
| `~/.config/omnixy/shell.json`    | the shell      | full layout + per-entry settings + enabled plugin list |
| `~/.config/omnixy/plugins/<id>/` | user           | drop-in third-party plugin source files                |

The `config/omnixy/shell.json` default config describes the
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      "left":   [ { "id": "omnixy.menu" }, { "id": "omnixy.workspaces" } ],
      "center": [ { "id": "omnixy.clock", "format": "HH:mm" } ],
      "right": [
        { "id": "omnixy.audio" }
      ]
    }
...
1. **The active bar option is `bar.id`.** Omit it or set it to `omnixy.bar`
...
4. **Built-in widget ids are namespaced.** Use ids such as `omnixy.clock`,
   `omnixy.audio`, and `omnixy.network`. The migration rewrites older ids
...
6. **Multiple instances** are allowed when a manifest sets
   ...
   widgets in different timezones are just two `{"id":"omnixy.clock", "timezone": ...}`
...
- Phase 1 — `omnixy-shell phase 1: host the existing bar in a single shell`
- Phase 2 — `omnixy-shell phase 2: plugin registry and bar widget registry`
- Phase 3 — `omnixy-shell phase 3: fold bar-settings into the shell as a panel plugin`
- Phase 4 — `omnixy-shell phase 4: absorb background-switcher as a plugin`
- Phase 5 — `omnixy-shell phase 5: docs, cleanup, and migration crumbs`
- Phase 6 — `omnixy-shell phase 6: reviewer cleanup (path traversal, collision, races)`
- Phase 7 — `omnixy-shell phase 7: replace socket with IpcHandler, rename to image-picker`
- Phase 8a — `omnixy-shell phase 8a: unified shell.json with inline plugin settings`
