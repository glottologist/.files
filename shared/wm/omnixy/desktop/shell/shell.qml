...
ShellRoot {
  ...
  // The omnixy-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. OMNIXY_PATH is provided by the uwsm
  ...
  readonly property string shellPath: omnixyPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: omnixyPath + "/config/omnixy/shell.json"
  readonly property string userConfigPath: home + "/.config/omnixy/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        ...
        center: [{ id: "omnixy.clock", format: "dddd HH:mm" }],
        right: [{ id: "omnixy.audio" }]
      }
    },
    plugins: []
  })
  ...
  Component.onCompleted: {
    console.log("omnixy-shell paths",
      "omnixyPath=" + shell.omnixyPath,
  ...
  readonly property string defaultBarId: "omnixy.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("omnixyPath" in target) target.omnixyPath = shell.omnixyPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("omnixyPath" in inst) inst.omnixyPath = shell.omnixyPath
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. omnixy.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("omnixyPath" in item) item.omnixyPath = shell.omnixyPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["omnixy.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("omnixy.image-picker", payload) ? "ok" : "unknown"
    }
    ...
    function cancel(doneFile: string): string {
      ...
      if (picker && typeof picker.closeSelector === "function") {
        ...
        shell.hide("omnixy.image-picker")
      }
      return "ok"
    }
  ...
  IpcHandler {
    ...
    function listPlugins(): string {
      ...
      for (var id in plugins) {
        ...
        var metadata = plugins[id].omnixy
        var clonedFrom = Util.isPlainObject(metadata) ? String(metadata.clonedFrom || "") : ""
        out.push({
          id: id,
          name: plugins[id].name,
          kinds: kinds,
          // What `omnixy plugin enable/disable` toggles: for a widget that is
