...
// omnixy-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/omnixy/agents/usage"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["omnixy-agent-usage-update"]
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/omnixy/agents-disabled.json"
  ...
  // Snapshots keep the field names older Omnixy versions wrote, so a fleet
