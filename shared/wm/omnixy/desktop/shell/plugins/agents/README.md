...
`omnixy-agent-usage-update` writes to `~/.local/state/omnixy/agents/usage/`
...
`omnixy plugin disable omnixy.agents`.

## Data

Each agent is one JSON record in `~/.local/state/omnixy/agents/usage/`,
written by `omnixy-agent-usage-update`. That command runs one
`omnixy-agent-usage-<agent>` collector per agent; the widget invokes it
...
configuration in `~/.config/omnixy/agents/fireworks.json`:
...
- IPC: `omnixy-shell omnixy.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/omnixy/shell.json`. The
top-level keys can be set with
`omnixy bar set omnixy.agents <key> <value>`:
...
```bash
omnixy bar set omnixy.agents refreshIntervalSec 300 --json
omnixy bar set omnixy.agents syncDir '~/Sync/agent-usage'
```
...
```bash
omnixy bar set omnixy.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
