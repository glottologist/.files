...
// omnixy-network-qr, which emits the interface, security, and SSID it
...
Item {
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omnixy.wifiqr")
    else close()
  }
  ...
  function generate(requestedIface) {
    ...
    qrProc.command = requestedIface
      ? ["omnixy-network-qr", "--meta", requestedIface]
      : ["omnixy-network-qr", "--meta"]
    qrProc.running = true
  }
  ...
  function togglePassword() {
    ...
    pwProc.command = ["omnixy-network-password", iface]
    pwProc.running = true
  }
  ...
  PanelWindow {
    ...
    WlrLayershell.namespace: "omnixy-network-qr"
