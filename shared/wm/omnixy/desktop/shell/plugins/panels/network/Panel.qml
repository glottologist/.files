...
Panel {
  id: root
  moduleName: "omnixy.network"
  ipcTarget: "omnixy.network"
  ...
  // `omnixy-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `omnixy-network-band`. `bandCurrent` is the band
  ...
  // The share card is its own panel plugin (omnixy.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("omnixy.wifiqr", JSON.stringify(payload))
  }
  ...
  function setBand(band) {
    ...
    actionProc.command = ["omnixy-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (omnixy.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("omnixy.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "omnixy-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "omnixy-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["omnixy-network-status", "--verbose"]
