...
Panel {
  id: root
  moduleName: "omnixy.monitor"
  ipcTarget: "omnixy.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (omnixy-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["omnixy-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("omnixy.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "omnixy-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["omnixy-display-text-size", String(px)]
  ...
  Process {
    ...
    // `omnixy-brightness-display` races the hardware/driver and can
