# Tailscale Omnixy Widget

Native Omnixy bar widget for Tailscale.
...
`omnixy-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `omnixy tailscale receive`
...
This widget ships as first-party plugin `omnixy.tailscale`. Add it with `omnixy plugin enable omnixy.tailscale`, then place it with `omnixy bar move omnixy.tailscale` if desired.
