...
Panel {
  id: root
  moduleName: "omnixy.weather"
  ipcTarget: "omnixy.weather"
  manageIpc: false
  ...
  // omnixy-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/omnixy/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["omnixy-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["omnixy-weather-location", "--set", name]
    else
      locationSaveProc.command = ["omnixy-weather-location", "--clear"]
  ...
  // Mirrors omnixy-weather-icon's wttr.in code → nerd-font glyph mapping.
