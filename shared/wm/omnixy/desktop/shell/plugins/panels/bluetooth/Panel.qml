...
Panel {
  id: root
  moduleName: "omnixy.bluetooth"
  ipcTarget: "omnixy.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/omnixy-bluetooth-device;
  ...
  function deviceCommand(action, address) {
    return ["omnixy-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via omnixy-bluetooth-device.
  ...
  // the adapter came back on at the next boot. omnixy-bluetooth-power moves the
