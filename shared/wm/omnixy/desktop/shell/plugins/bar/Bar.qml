...
Item {
  ...
  // The omnixy-shell host injects omnixyPath from OMNIXY_PATH.
  required property string omnixyPath
  ...
  property string omnixyConfigDir: home + "/.config/omnixy"
  ...
  // to "monospace"), so changing the system font (via `omnixy-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "omnixy.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, omnixyConfigDir)
  ...
  FileView {
    path: root.stateHome + "/omnixy/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `omnixy-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/omnixy/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/omnixy/toggles"
    watchChanges: true
    printErrors: false
  ...
  // shell restarts. `omnixy-toggle-bar` nudges this after flipping the flag
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "omnixy-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "omnixy-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
