...
BarWidget {
  ...
  // UseQApplication` - omnixy's shell.qml does not, so every submenu click was
  ...
  function ownedByOmnixy(item) {
    var layout = root.bar && root.bar.layoutConfig ? root.bar.layoutConfig : null
    return TrayModel.ownedByOmnixy(item, layout)
  }
  ...
  function bucket(category) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (ownedByOmnixy(item)) continue
      if (category === "all") {
        result.push(item)
        continue
      }
  ...
  function persistTrayState(pinned, hidden) {
    ...
    var id = root.moduleName || "omnixy.tray"
