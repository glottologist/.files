...
Item {
  ...
  // Injected by omnixy-shell (the first-party service loader).
  property var shell: null

  readonly property string home: Quickshell.env("HOME")
  readonly property string stayAwakeStateDir: home + "/.local/state/omnixy/indicators"
  ...
  readonly property string screensaverClass: "org.omnixy.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("omnixy idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(omnixy-shell lock isLocked 2>/dev/null) == \"true\" ]] || omnixy-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "omnixy-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "omnixy-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/omnixy/indicators\" && touch \"$HOME/.local/state/omnixy/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/omnixy/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/omnixy/indicators\"; if [[ -f $HOME/.local/state/omnixy/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
