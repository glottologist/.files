...
// against `omnixy-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "omnixy-channel-current",
  "omnixy-default-agent",
  "omnixy-default-browser",
  "omnixy-default-editor",
  "omnixy-default-terminal",
  "omnixy-dns"
]
...
function guardHelpers() {
  return 'declare -A __omnixy_pkgs=()\n'
    + 'mapfile -t __omnixy_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __omnixy_pkg in "${__omnixy_pkg_names[@]}"; do __omnixy_pkgs[$__omnixy_pkg]=1; done\n'
    + '__omnixy_pkg_has() { [[ -n ${__omnixy_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'omnixy-pkg-present() { local p; for p in "$@"; do __omnixy_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'omnixy-pkg-missing() { local p; for p in "$@"; do __omnixy_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'omnixy-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'omnixy-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__omnixy_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
