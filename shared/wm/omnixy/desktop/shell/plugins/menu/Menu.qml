...
Item {
  ...
  // Injected by omnixy-shell when this plugin is summoned.
  property string omnixyPath: Quickshell.env("OMNIXY_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `omnixy-shell shell summon omnixy.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: omnixyPath + "/default/omnixy/omnixy-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/omnixy/extensions/omnixy-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(omnixy-font-current 2>/dev/null); omnixy-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "omnixy-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); omnixy-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "omnixy-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `omnixy-shell shell summon omnixy.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `omnixy menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/omnixy/extensions/omnixy-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omnixy-menu"
