...
function glyphFromHints(hints) {
  return stringHint(hints, "omnixy-glyph")
}

// The click action: a JSON argv string from omnixy-notification-send
...
function execArgvFromHints(hints) {
  return stringHint(hints, "omnixy-exec-argv")
}

// Validate a persisted omnixy-exec-argv into a runnable argv, or null. This is
...
// ~/.local/state/omnixy/notifications/ so toasts survive shell restarts
// (e.g. the restart `omnixy-update` performs). The file exists exactly as
...
// senders (all Omnixy web apps) delete their scoped /tmp files on close,
