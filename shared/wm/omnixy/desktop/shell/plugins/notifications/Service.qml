// Notification service for the omnixy shell.
...
Item {
  ...
  readonly property string stateDir: home + "/.local/state/omnixy/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the omnixy bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "omnixy-notifications"
  ...
  // and rare.
  //   - omnixy-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always omnixy or system shell scripts —
  ...
  //   - app_name is "omnixy-action" (Omnixy's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. Omnixy's own toasts carry the
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.omnixyPath + "/bin/omnixy-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  // toasts survive shell restarts (notably the restart `omnixy-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "omnixy-notifications"
