...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omnixy.emojis")
  }
  ...
  function applySelected(emoji) {
    if (!emoji) return
    root.dismiss()
    Quickshell.execDetached([root.omnixyPath + "/bin/omnixy-menu-emoji-insert", emoji])
  }

  ListModel { id: displayModel }

  FileView {
    path: root.omnixyPath + "/shell/plugins/emojis/emojis.json"
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omnixy-emojis"
