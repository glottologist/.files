...
Item {
  ...
  readonly property string currentBackgroundLink: stateHome + "/omnixy/current/background"
  ...
  function logEvent(event) {
    ...
    console.log("omnixy lock " + lastEventAt + " " + event)
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omnixy-lock-preview"
  ...
  PamContext {
    id: fingerprintPam
    config: "omnixy-lock-fingerprint"
    user: root.userName
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/omnixy-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    ...
    command: ["bash", "-c", "omnixy-hyprland-session-locked"]
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "omnixy-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "omnixy-brightness-keyboard off; omnixy-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/omnixy-lock-password"
    watchChanges: true
    printErrors: false
