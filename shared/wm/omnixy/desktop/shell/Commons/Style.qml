...
QtObject {
  ...
  // component follows the fontconfig alias `omnixy-font-set` writes.
  ...
  // alias path keeps working when the user runs `omnixy font set`), but
  ...
  // OMNIXY_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("OMNIXY_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `omnixy font
  ...
  // `omnixy toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/omnixy/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
