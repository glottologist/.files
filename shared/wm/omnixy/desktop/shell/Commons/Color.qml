...
QtObject {
  ...
  readonly property string currentThemePath: stateHome + "/omnixy/current/theme"
  ...
  // Style. User keys win, so a machine-level `~/.config/omnixy/shell.toml`
  ...
  // is where `omnixy display text size` writes `[font] base-size`. Watched so the
  // CLI takes effect live without restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/omnixy/shell.toml"
    watchChanges: true
    printErrors: false
