...
Item {
  ...
  function remove(desktopId, name) {
    var id = String(desktopId || "")
    if (!id) return
    Util.execDetached(Util.shellQuote(root.omnixyPath + "/bin/omnixy-remove-launcher-entry") + " " + Util.shellQuote(id) + " " + Util.shellQuote(String(name || id)))
  }
  ...
  function hiddenEntryScanCommand() {
    ...
    var script = root.omnixyPath + "/shell/services/hidden-entries.sh"
  ...
  function closeLaunchFeedback(serial) {
    ...
    if (root.launchOsdOpen) {
      Quickshell.execDetached(["omnixy-shell", "osd", "close"])
  ...
  FileView {
    path: root.omnixyPath + "/default/omnixy/launcher.hides"
    watchChanges: true
    printErrors: false
  ...
  Timer {
    ...
    onTriggered: {
      ...
      Quickshell.execDetached(["omnixy-shell", "osd", "show", JSON.stringify({ icon: "󱓞", message: root.launchOsdMessage, duration: 0 })])
    }
  }
