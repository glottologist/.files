...
QtObject {
  ...
  property string pluginsDir: home + "/.config/omnixy/plugins"
  ...
  //   - the built-in bar option (`omnixy.bar`) is active when `bar.id` is
  //     missing or set to `omnixy.bar`.
  ...
  //     silently make `omnixy launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "omnixy.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.omnixy) ? manifest.omnixy : null
  ...
  // back, and so a plugin that is both a widget and a menu (omnixy.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "omnixy.workspaces", center: "omnixy.weather", right: "omnixy.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.omnixy)
        ? candidateManifest.omnixy : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "omnixy.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "omnixy.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `omnixy.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("omnixy.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party Omnixy plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/omnixy/plugins.
    var script = ""
